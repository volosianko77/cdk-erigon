...
func (c *Config) CheckCompatible(newcfg *Config, height uint64) *chain.ConfigCompatError {
	...
	var lasterr *chain.ConfigCompatError
	for {
...
func newCompatError(what string, storedblock, newblock *big.Int) *chain.ConfigCompatError {
	var rew *big.Int
	switch {
	...
	err := &chain.ConfigCompatError{What: what, StoredConfig: storedblock, NewConfig: newblock, RewindTo: 0}
